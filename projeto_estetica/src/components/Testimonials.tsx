    ...
    <section id="testimonials" className="py-20 bg-garage-dark">
      <div className="max-w-6xl mx-auto px-4">
        <h2 className="text-3xl md:text-4xl font-bold text-center mb-4">
          Avaliações dos <span className="text-garage-red">Nossos Clientes</span>
        </h2>
            ...
            {reviews.map((review) => (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <span className="w-10 h-10 rounded-full bg-garage-red text-black font-bold flex items-center justify-center">
                    {review.name.charAt(0)}
                  </span>
                  <span className="flex gap-0.5">
                    {Array.from({ length: review.rating }).map((_, i) => (
                      <svg key={i} className="w-4 h-4 text-garage-red" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
                    ))}
                  </span>
                </div>
                <p className="text-garage-muted flex-1 mb-5">&ldquo;{review.text}&rdquo;</p>
        ...
        <div className="mt-12 text-center">
          <a
            ...
            className="inline-flex items-center px-8 py-4 bg-garage-red hover:bg-garage-red-hover text-black font-semibold rounded-lg transition-colors shadow-lg hover:shadow-xl transform hover:scale-105"
          >
